...
export default function Test({ theme, onToggleTheme }) {
    ...
    const fetchQuestions = async () => {
      try {
        ...
        if (questionSet) {
          ...
          data = await questionAPI.getQuestionsByCategoryAndSet(categoryToUse, questionSet);
        } else {
          // Old way: fetch by category only (for backward compatibility)
          data = await questionAPI.getQuestionsByCategory(categoryToUse);
        }
        setQuestions(data);
      } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-lg text-center max-w-md">
          ...
          <p className="mb-6 text-slate-600 dark:text-slate-400">No questions found for this test type.</p>
          <button
  ...
  const handleFinish = async () => {
    ...
    if (responses.length > 0) {
      setSubmitting(true);
      try {
        await responseAPI.submitBulk(responses);
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-5 sm:p-8">
              ...
              <div className="flex items-start justify-between mb-5 sm:mb-6">
                <div className="flex-1">
                  ...
                  <p className="text-base sm:text-lg text-slate-800 dark:text-slate-100 leading-relaxed">{currentQuestion.content}</p>
                </div>
              </div>
              ...
              <div className="space-y-2.5 sm:space-y-3 mb-5 sm:mb-6">
                {[...currentQuestion.answers].sort((a, b) => a.id - b.id).map((answer, idx) => {
                  ...
                  let statusClass = 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/50';
                  ...
                  if (isRevealed) {
                    ...
                    if (isCorrect) {
                      statusClass = 'border-green-500 dark:border-green-600 bg-green-50 dark:bg-green-900/30';
                      ...
                      statusClass = 'border-red-500 dark:border-red-600 bg-red-50 dark:bg-red-900/30';
                    ...
                    statusClass = 'border-blue-500 dark:border-blue-600 bg-blue-50 dark:bg-blue-900/30';
                  ...
                  return (
                    <button
                      ...
                      disabled={isRevealed}
                      className={`w-full text-left p-3 sm:p-4 rounded-xl border-2 transition-all duration-200 ${statusClass} ${isRevealed ? 'cursor-default' : 'cursor-pointer group'}`}
                    >
                      <div className="flex items-center gap-3 sm:gap-4">
                        <div className={`flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center text-sm sm:text-base font-semibold ${badgeClass}`}>
                          {String.fromCharCode(65 + idx)}
                        </div>
                        <span className="flex-1 text-sm sm:text-base text-slate-700 dark:text-slate-200">{answer.content}</span>
              ...
              {isRevealed && currentQuestion.answers.some(a => a.is_correct && a.explanation) && (
          ...
          <div className="lg:col-span-1 space-y-4 sm:space-y-6">
            {/* Summary Card */}
            <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-4 sm:p-6">
              <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-3 sm:mb-4 text-base sm:text-lg">Progress</h3>
              <div className="space-y-2.5 sm:space-y-3">
                <div className="flex justify-between text-xs sm:text-sm">
                  <span className="text-slate-600 dark:text-slate-400">Answered</span>
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{answeredCount} / {questions.length}</span>
                </div>
                <div className="flex justify-between text-xs sm:text-sm">
                  <span className="text-slate-600 dark:text-slate-400">Flagged</span>
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{Object.values(flags).filter(Boolean).length}</span>
                </div>
              </div>
              <button
            ...
            {/* Question Navigator */}
            <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-4 sm:p-6">
              <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-3 sm:mb-4 text-base sm:text-lg">Questions</h3>
              <div className="grid grid-cols-5 gap-2">
                {questions.map((q, idx) => {
                  ...
                  let bgClass = 'bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300';
                  if (isCurrentQ) {
                    ...
                    bgClass = 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-900/70';
                  }
