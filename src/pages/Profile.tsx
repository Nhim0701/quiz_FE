    ...
    const fetchData = async () => {
      try {
        const [categoriesData, dashboard] = await Promise.all([
          questionAPI.getCategoriesWithSets(),
          responseAPI.getDashboard(),
        ]);

        console.log('Categories with sets fetched:', categoriesData);
        ...
        setDashboardData({ overall: { total_answered: 0, total_correct: 0, total_wrong: 0, overall_accuracy: 0 }, by_category: [], recent_activity: [] });
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white dark:bg-slate-800 shadow-sm rounded-xl p-4 sm:p-6">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <ThemeToggle theme={theme} onToggle={onToggleTheme} className="flex-1 sm:flex-none" />
            <button
        ...
        {dashboardData?.overall && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mb-1">Total Answered</p>
                  <p className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-slate-100 truncate">{dashboardData.overall.total_answered}</p>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mb-1">Correct Answers</p>
                  <p className="text-2xl sm:text-3xl font-bold text-green-600 dark:text-green-400 truncate">{dashboardData.overall.total_correct}</p>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mb-1">Wrong Answers</p>
                  <p className="text-2xl sm:text-3xl font-bold text-red-600 dark:text-red-400 truncate">{dashboardData.overall.total_wrong}</p>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mb-1">Accuracy</p>
                  <p className="text-2xl sm:text-3xl font-bold text-indigo-600 dark:text-indigo-400 truncate">{dashboardData.overall.overall_accuracy}%</p>
                </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-4 sm:p-6">
            <h2 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4 sm:mb-6">Available Tests</h2>

            {categoriesWithSets && categoriesWithSets.length > 0 ? (
              <div className="space-y-6 sm:space-y-8">
                {categoriesWithSets.map((categoryData) => (
                  <div key={categoryData.category} className="border-b border-slate-200 dark:border-slate-700 last:border-0 pb-6 sm:pb-8 last:pb-0">
                    <div className="flex items-center justify-between mb-3 sm:mb-4">
                      <div>
                        <h3 className="text-lg sm:text-xl font-bold text-slate-800 dark:text-slate-100 capitalize">{categoryData.category}</h3>
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">{categoryData.total_questions} total questions</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                      {categoryData.question_sets.map((set) => (
                        <button
                          ...
                          onClick={() => handleStartTest(categoryData.category, set.question_set)}
                        ...
                        >
                          ...
                          <h4 className="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-100 mb-1 truncate">{set.question_set}</h4>
                          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 truncate">Questions {set.question_range}</p>
              ...
              <div className="text-center py-8 sm:py-12">
                <svg className="w-12 h-12 sm:w-16 sm:h-16 text-slate-300 dark:text-slate-600 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <p className="text-base sm:text-lg text-slate-600 dark:text-slate-300 mb-2">No tests available</p>
                <p className="text-xs sm:text-sm text-slate-400 dark:text-slate-500">Please add questions to the database first</p>
              </div>
            )}
          </div>
          ...
          {dashboardData?.by_category && dashboardData.by_category.length > 0 && (
            <div className="bg-white dark:bg-slate-800 shadow-sm rounded-xl p-6">
              <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4">Performance by Category</h2>
              <div className="space-y-3">
                {dashboardData.by_category.map((stat) => (
                  <div key={stat.category} className="border-b border-slate-100 dark:border-slate-700 last:border-0 pb-3 last:pb-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{stat.category}</span>
                      <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{stat.accuracy}%</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                      <span className="text-green-600 dark:text-green-400">{stat.correct_answers} correct</span>
                      <span>•</span>
                      <span className="text-red-600 dark:text-red-400">{stat.wrong_answers} wrong</span>
                    </div>
              ...
              {dashboardData?.recent_activity && dashboardData.recent_activity.length > 0 ? (
                <div className="space-y-3">
                  {dashboardData.recent_activity.map((activity) => (
                    <div key={activity.id} className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
                        activity.is_correct ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30'
                      }`}>
                        {activity.is_correct ? (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-slate-700 dark:text-slate-300 line-clamp-2">{activity.question_preview}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">No activity yet. Start a test to see your progress!</p>
                </div>
              )}
            </div>
          </div>
